...
export default function AdminPage() {
  ...
  // Compact UI is DEFAULT ON now (persisted)
  const [compact, ] = useState<boolean>(() => {
    try {
      const v = localStorage.getItem('bdz_compact')
  ...
  // ensure persisted
  useEffect(() => {
    try { localStorage.setItem('bdz_compact', compact ? '1' : '0') } catch {}
  }, [compact])
  ...
  async function bulkConfirm() {
    ...
    try {
      ...
      selectedIds.forEach((id) => {
        batch.update(doc(db, 'orders', id), { status: 'confirmed' } as any)
      })
      await batch.commit()
  ...
  /* ---------------- size helpers based on compact (default true) ---------------- */
  const btnSize = compact ? 'text-sm px-2 py-1' : 'text-sm px-3 py-1.5'
  const inputSize = compact ? 'text-sm px-2 py-1' : 'text-sm px-3 py-1.5'
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900">
      <div className="max-w-screen-xl mx-auto px-4 py-6">
        ...
        {/* Header */}
        <header className="bg-white rounded-xl shadow p-4 md:p-5 mb-4">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
            ...
            <div className="flex flex-col items-end gap-3">
              <div className="flex items-center space-x-2">
                {/* compact is default; control button removed as requested */}
                {user && (
                  <button onClick={logout} className={`${btnSize} rounded bg-red-600 text-white`}>
                    Logout
                  </button>
                )}
              </div>
              ...
              <div className="flex gap-2 items-center">
                ...
                <div className="bg-emerald-50 px-3 py-1 rounded text-sm">
          ...
          {/* secondary toolbar */}
          {user && (
            <div className={`mt-3 flex flex-wrap items-center ${gapSize} ${compact ? 'py-2' : 'py-3'}`}>
              <div className={`flex items-center bg-gray-100 rounded ${inputSize} flex-1 min-w-[150px] max-w-md`}>
                ...
                <input
                  placeholder="Search product / name / phone / address"
                  value={searchTerm}
              ...
              <select value={statusFilter} onChange={(e)=>setStatusFilter(e.target.value as any)} className={`rounded border ${inputSize} bg-white`}>
              ...
              <div className="flex items-center gap-2">
                <input type="date" value={dateFrom} onChange={(e)=>setDateFrom(e.target.value)} className={`rounded border ${inputSize}`} />
                <input type="date" value={dateTo} onChange={(e)=>setDateTo(e.target.value)} className={`rounded border ${inputSize}`} />
                {(dateFrom||dateTo) && <button onClick={()=>{setDateFrom(''); setDateTo('')}} className="text-xs underline text-gray-600">Clear</button>}
              </div>

              <button onClick={fetchOrders} className={`rounded ${btnSize} bg-blue-50 text-blue-700`}>Refresh</button>
              <button onClick={downloadCsv} className={`rounded ${btnSize} bg-emerald-600 text-white`}>CSV ⬇</button>
                      ...
                      <tr key={o.id} className="hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4 text-center">
                          <div className="flex justify-center gap-2">
                            {o.status !== 'confirmed' && (
                              <button onClick={() => confirmOrder(o.id)} className="px-3 py-1 bg-green-600 text-white rounded text-sm">Confirm</button>
                            )}
                            <button onClick={() => removeOrder(o.id)} className="px-3 py-1 bg-red-600 text-white rounded text-sm">Delete</button>
                          </div>
                        </td>
                      </tr>
                    )
